...
const CourseUnitFilter: React.FC<CourseUnitFilterProps> = ({ 
  ...
  useEffect(() => {
    if (selectedCourse && selectedUnit) {
      const unitBelongsToCourse = units.find(unit => 
        unit.id === selectedUnit && unit.course_id === selectedCourse
      );
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6 mb-6">
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-2 text-gray-600">Loading courses and units...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="relative">
          ...
          >
            <span className={selectedCourseData ? "text-gray-900" : "text-gray-500"}>
              {selectedCourseData ? `${selectedCourseData.code} - ${selectedCourseData.name}` : "Select a course"}
            </span>
              ...
              {courses.map(course => (
                <button
                  ...
                  className={`w-full text-left p-3 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${
                    selectedCourse === course.id ? 'bg-rose-50 text-rose-700' : ''
                  }`}
                >
                  <div>
        ...
        <div className="relative">
          ...
          >
            <span className={selectedUnitData ? "text-gray-900" : "text-gray-500"}>
              {!selectedCourse 
              ...
              {filteredUnits.map(unit => (
                ...
                >
                  <div>
                    ...
                    <div className="text-sm text-gray-600">{unit.unit_name}</div>
                  </div>
                </button>
              ))}
            </div>
...
const UploadForm: React.FC<UploadFormProps> = ({ courses, units, onUpload, onCancel }) => {
...
const Page: React.FC = () => {
  ...
  // Sample resources for initial state (replace or remove as needed)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch courses and units in parallel
  ...
  const filteredResources = resources.filter(resource => {
    ...
    const matchesCourse = !selectedCourse || resource.course === selectedCourseData?.name;
    
    // Filter by unit
    const selectedUnitData = units.find(u => u.id === selectedUnit);
    const matchesUnit = !selectedUnit || resource.unit === selectedUnitData?.unit_name;
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col lg:ml-64">
        ...
        <main className="flex-1 p-4 lg:p-6 max-w-7xl mx-auto w-full">
          ...
          {/* Display loaded data info for debugging */}
          {!loading && (
            <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="text-sm text-blue-700 space-y-1">
                ...
                <p>Selected Course: {selectedCourse ? courses.find(c => c.id === selectedCourse)?.name : 'None'}</p>
                <p>Selected Unit: {selectedUnit ? units.find(u => u.id === selectedUnit)?.unit_name : 'None'}</p>
              </div>
            </div>
          )}
        </main>
