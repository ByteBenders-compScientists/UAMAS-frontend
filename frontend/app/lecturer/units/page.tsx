  ...
  >
    ...
    <div className="space-y-3">
      <div className="flex items-center text-sm text-gray-600">
        ...
        <span>
          {unit.course?.name || 'Unknown Course'} ({unit.course?.code || 'N/A'})
        </span>
      </div>
...
export default function UnitsPage() {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        const res = await fetch("https://api.waltertayarg.me/api/v1/auth/lecturer/units", {
          method: "GET",
          headers: {
        ...
        if (!res.ok) {
          ...
          throw new Error(`HTTP ${res.status}: Failed to fetch units`)
        }
        ...
        if (Array.isArray(data)) {
          setUnits(data)
          console.log("Set units:", data.length, "items") // Debug log
        } else {
        ...
        setUnits([]) // Always set empty array on error
      } finally {
        setIsLoading(false)
      }
    }
  ...
  // Safe filter - ensure units is always an array
  ...
  const handleAddUnit = async (unitData: any) => {
    try {
      if (selectedUnit) {
        ...
        const response = await fetch(`https://api.waltertayarg.me/api/v1/auth/lecturer/units/${selectedUnit.id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        // Add new unit
        const response = await fetch("https://api.waltertayarg.me/api/v1/auth/lecturer/units", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      const unitsResponse = await fetch("https://api.waltertayarg.me/api/v1/auth/lecturer/units", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      try {
        ...
        console.error("Error reloading units:", reloadError)
        setUnits([])
      }
    }
  }
  ...
  const handleDeleteUnit = async (id: string) => {
    ...
    try {
      const response = await fetch(`https://api.waltertayarg.me/api/v1/auth/lecturer/units/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
